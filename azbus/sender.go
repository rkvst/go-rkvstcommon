...
func (*Sender) updateSendingMessage(ctx context.Context, sp Spanner, log Logger, message *OutMessage) Logger {
	...
	attrs, log, err = sp.InjectContext(ctx, log)
	if err != nil {
		log.Infof("updateSendingMessage(): failed to inject: %v", err)
	}
	for k, v := range attrs {
...
func (s *Sender) Send(ctx context.Context, message *OutMessage) error {
	...
	// get trace id into properties if not already there.
	OutMessageSetPropertyFromMetadata(ctx, message, TraceID)
